...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    recursiveTrending(this, new EmptyTrending())
  }

  def recursiveTrending(set : TweetSet, trending: Trending): Trending = {
    if(set.isEmpty) return trending
    val min = set.findMin
    recursiveTrending(set.remove(min), trending + min)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(this.head)) return this.tail.filter0(p, accu.incl(head))
  ...
  def union(that: TweetSet): TweetSet =
    ...
    else if(contains(that.head)) return union(that.tail)
    else return incl(that.head).union(that.tail)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists((a) => tw.text.contains(a)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists((a) => tw.text.contains(a)))
